...
export default function PublicVehicleQRPage({ params }: { params: Promise<{ secureVehicleId: string }> }) {
  ...
  useEffect(() => {
    setMounted(true);
    const v = mockStorage.getVehicleById(resolvedParams.secureVehicleId);
    if (v) {
      setVehicle(v);
  ...
  }, [resolvedParams.secureVehicleId]);
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      ...
      {/* Contextual Header matching UIUX vehicle_found_kl_16_p_78/code.html */}
      ...
      {/* Vehicle Detail Card matching UIUX */}
      ...
      <form onSubmit={handleCalculateAndSubmit} className="bg-surface p-5 rounded-xl border border-outline-variant shadow-sm space-y-4">
        ...
        {/* Real-time Calculation Summary Box (Matching UIUX payment_summary) */}
        {calcPreview && (
